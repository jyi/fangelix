DEFAULT_MODULES = llvm-gcc llvm2 minisat stp klee-uclibc klee z3 clang bear runtime frontend maxsmt synthesis nsynth
OPTIONAL_MODULES = semfix
...
LLVM6_URL=https://releases.llvm.org/6.0.0/llvm-6.0.0.src.tar.xz
...
PATCH_DIR=$(ANGELIX_ROOT)/patches
...
llvm2: build/$(LLVM2_ARCHIVE) build/$(LLVM2_PATCH)
	...
	cd $(LLVM2_DIR) && patch -p1 -N < $(PATCH_DIR)/llvm-2.9.patch
	cd $(LLVM2_DIR) && ./configure --enable-optimized --enable-assertions && make -j $(NUM_OF_CPU)
...
# klee:
#	cd $(KLEE_DIR) && ./configure --with-llvm=$(LLVM2_DIR) --with-stp=$(STP_DIR)/build --with-uclibc=$(KLEE_UCLIBC_DIR) --enable-posix-runtime \
#	&& patch -p1 -N < $(PATCH_DIR)/klee.patch \
#	&& make ENABLE_OPTIMIZED=1 -j $(NUM_OF_CPU)

klee: cJSON
	cd $(KLEE_DIR) && ./configure --with-llvm=$(LLVM2_DIR) --with-stp=$(STP_DIR)/build --with-uclibc=$(KLEE_UCLIBC_DIR) --enable-posix-runtime \
	&& make ENABLE_OPTIMIZED=1 VERBOSE=1 REQUIRES_RTTI=1 --trace -j $(NUM_OF_CPU)

clean-klee:
	cd $(KLEE_DIR) && make clean

zesti:
	cd $(ZESTI_DIR) && ./configure --with-llvm=$(LLVM2_DIR) --with-stp=$(STP_DIR)/build --with-uclibc=$(KLEE_UCLIBC_DIR) --enable-posix-runtime \
	&& make ENABLE_OPTIMIZED=1 -j $(NUM_OF_CPU) VERBOSE=1

clean-zesti:
	cd $(ZESTI_DIR) && make clean

katch:
	cd $(KATCH_DIR) && ./configure --with-llvm=$(LLVM2_DIR) --with-stp=$(STP_DIR)/build --with-uclibc=$(KLEE_UCLIBC_DIR) --enable-posix-runtime \
	&& make ENABLE_OPTIMIZED=1 -j $(NUM_OF_CPU) VERBOSE=1

clean-katch:
	cd $(KATCH_DIR) && make clean

# Cjson

cJSON:
	cd src/cJSON && make -j $(NUM_OF_CPU)
	mkdir -p $(ANGELIX_ROOT)/build/lib/cJSON
	cp src/cJSON/libcjson.a $(ANGELIX_ROOT)/build/lib/cJSON/
	cp src/cJSON/libcjson_utils.a $(ANGELIX_ROOT)/build/lib/cJSON/
	cp src/cJSON/libcjson.klee.a $(ANGELIX_ROOT)/build/lib/cJSON/
	cp src/cJSON/libcjson_utils.klee.a $(ANGELIX_ROOT)/build/lib/cJSON/
	cd src/cJSON && make clean

# Angelix runtime #

runtime: cJSON
	cd src/runtime && make -j $(NUM_OF_CPU)
...
z3: $(Z3_DIR)
	cd $(Z3_DIR) && python scripts/mk_make.py --java && cd build && make -j $(NUM_OF_CPU)
...
clang: build/$(LLVM3_ARCHIVE) build/$(CLANG_ARCHIVE) build/$(COMPILER_RT_ARCHIVE)
	...
	cd $(LLVM3_DIR) && patch -p1 -N < $(PATCH_DIR)/llvm-3.7.0.src.patch
	mkdir -p "$(LLVM3_DIR)/build" && cd "$(LLVM3_DIR)/build" && cmake -G "Unix Makefiles" ../ && make -j $(NUM_OF_CPU) VERBOSE=2

build/$(LLVM3_ARCHIVE):
	cd build && $(DOWNLOAD) $(LLVM3_URL)

build/$(LLVM6_ARCHIVE):
	cd build && $(DOWNLOAD) $(LLVM6_URL)
...
frontend: $(LLVM3_DIR)/tools/clang/tools/angelix
	...
	cd "$(LLVM3_DIR)/build" && make -j $(NUM_OF_CPU)
	...
	cp "$(LLVM3_DIR)/build/bin/mutate" "$(ANGELIX_ROOT)/build/bin"
