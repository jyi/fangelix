#! /usr/bin/env python
# Customized from Delta Debugging code written by Andreas Zeller
...
class DD:
    ...
    def _dd(self, c, n):
        ...
        logger.debug('granularity: {}'.format(n))
        assert self.test([]) == self.FAIL
        ...
        while True:
            tc = self.test(c)
            assert tc == self.PASS or tc == self.UNRESOLVED
            ...
            config = reduce((lambda x, y: '{} + {}'.format(x, y)), map(len, cs))
            logger.info('(run #{}): trying {}'.format(repr(run), config))
            ...
            for i in range(n):
                if self.debug_dd:
                    logger.info('trying {}'.format(self.pretty(cs[i])))
                ...
                if t == self.PASS:
                    # Found
                    if self.debug_dd:
                        logger.info('found {} deltas: {}'.
                                    format(len(cs[i]), self.pretty(cs[i])))
            ...
            if not c_failed:
                ...
                for j in range(n):
                    ...
                    if t == self.PASS:
                        if self.debug_dd:
                            logger.info('reduced to {} deltas: {}'.
                                        format(len(cbars[i]), self.pretty(cbars[i])))
    ...
    def dddiff(self, c):
        ...
        if self.debug_dd:
            logger.info('dddiff({}, {})...'.format(self.pretty(c), repr(n)))
    ...
    def _dddiff(self, c1, c2, n):
        ...
        while 1:
            ...
            for j in range(n):
                ...
                if t == self.PASS and t1 == self.FAIL:
                    # Found
                    progress    = 1
                    next_c2     = csub
                    next_n      = 2
                ...
                if t == self.FAIL and t2 == self.PASS:
            ...
            if progress:
                ...
                next_n = min(len(c), n * 2)
                logger.info('increase granularity to {}'.format(next_n))
                cbar_offset = (cbar_offset * next_n) // n
